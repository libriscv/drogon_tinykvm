...
extern void remote_backend_function(std::pmr::polymorphic_allocator<std::byte>& alloc) __attribute__((noreturn));
...
{
	if constexpr (true) {
		void* addr = std::malloc(1024*8);
		remote_writeback(addr, 1024*8);
		...
		remote_backend_function(g_allocator);
