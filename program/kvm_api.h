 ...
 * @date 2023-10-28
 * 
 * Single-source-of-truth API header for KVM programs.
...
/* Streaming POST will receive each data segment as they arrive. A final POST
   ...
   right now the server will fill the VM with the whole POST data no matter what,
...
/**
 * Stream a response back to the server using streaming callback function.
...
/* Returns true (1) if this program is uploaded for the the purpose
   of live-debugging, and remotely debuggable. Otherwise, false (0). */
...
/* Fetch content from provided URL. Content will be allocated by the server
