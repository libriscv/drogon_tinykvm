...
static void usage(const char* progname)
{
	...
	fprintf(stderr, "  --snapshot-mode      Set snapshot profiling mode (none, accessed, reorder)\n");
	fprintf(stderr, "  --port <n>           Set listening port (default: 8080)\n");
...
static void init_settings(int argc, char** argv)
{
	...
	for (int i = 1; i < argc; ++i) {
		...
		if (arg == "--reservations" || arg == "-r") {
			...
			if (i + 1 < argc) {
				g_settings.port = std::stoi(argv[++i]);
			}
		} else if (arg == "--snapshot-mode") {
			if (i + 1 < argc) {
				std::string mode = argv[++i];
				if (mode == "none") {
					g_settings.snapshot_profiling_mode = SNAPSHOT_PROFILING_NONE;
				} else if (mode == "accessed") {
					g_settings.snapshot_profiling_mode = SNAPSHOT_PROFILING_ACCESSED;
				} else if (mode == "fault-order") {
					g_settings.snapshot_profiling_mode = SNAPSHOT_PROFILING_FAULT_ORDER;
				} else if (mode == "reorder") {
					g_settings.snapshot_profiling_mode = SNAPSHOT_PROFILING_REORDER;
				} else {
					fprintf(stderr, "Unknown snapshot profiling mode: %s\n", mode.c_str());
					usage(argv[0]);
				}
			}
...
int main(int argc, char** argv)
{
	...
	static const std::string DEFAULT_HOST = g_settings.host + ":" + std::to_string(g_settings.port);
