...
static void usage(const char* progname)
{
	...
	fprintf(stderr, "  --profiling          Enable profiling (default: false)\n");
...
int main(int argc, char** argv)
{
	...
	if (g_settings.concurrency > 0) {
		printf("* Tenant concurrency: %d (override)\n", g_settings.concurrency);
	} else {
		printf("* Tenant concurrency: hardware specified (%u)\n", std::thread::hardware_concurrency());
	}
