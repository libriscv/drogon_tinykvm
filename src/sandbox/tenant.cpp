...
static void configure_group(const std::string& name, kvm::TenantGroup& group, const It& obj)
{
	...
	else if (obj.key() == "dylink_address_hint")
	{
		if (obj.value().is_string()) {
			...
			group.dylink_address_hint = std::stoul(str, nullptr, 16);
		} else if (obj.value().is_number()) {
			// Treat as offset in megabytes
			group.dylink_address_hint = uint32_t(obj.value()) * 1048576ul;
		} else {
			throw std::runtime_error("Dylink address hint was not a number");
		}
	}
