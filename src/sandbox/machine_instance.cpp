...
void MachineInstance::reset_to(MachineInstance& source)
{
	...
	if (reset_needed) {
		...
		if (main_vm.m_store_state_on_reset) {
			...
			// Sort accessed pages by the order in page fault order, with kernel pages first
			const uint64_t kernel_end = main_vm.machine().kernel_end_address();
			// Build address-to-index map for O(1) lookup
			std::unordered_map<uint64_t, size_t> fault_order_map;
			fault_order_map.reserve(page_fault_order.size());
			for (size_t i = 0; i < page_fault_order.size(); ++i) {
				const uint64_t addr = page_fault_order[i];
				fault_order_map.insert_or_assign(addr, i);
			}
			const size_t not_faulted = std::numeric_limits<size_t>::max();
			std::sort(populate_pages.begin(), populate_pages.end(),
				[&] (const auto& a, const auto& b) {
					const bool a_kernel = a.first < kernel_end;
					const bool b_kernel = b.first < kernel_end;
					// Kernel pages before user pages
					if (a_kernel != b_kernel) {
						return a_kernel;
					}
					// Within same category, sort by fault order
					const auto a_it = fault_order_map.find(a.first);
					const auto b_it = fault_order_map.find(b.first);
					const size_t a_order = (a_it != fault_order_map.end()) ? a_it->second : not_faulted;
					const size_t b_order = (b_it != fault_order_map.end()) ? b_it->second : not_faulted;
