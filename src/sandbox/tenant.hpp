...
struct TenantGroup {
	...
	uint64_t dylink_address_hint = 0x200000; /* Image base address hint */
	uint64_t heap_address_hint = 0; /* Address hint for the heap */
	uint64_t max_storage_memory = 0; /* Megabytes, 0 = same as main memory */
	uint64_t storage_dylink_address_hint = 0x2000200000; /* Image base address hint for storage */
	...
	bool     storage_1_to_1 = false; /* Each request VM has its own storage VM */
	bool     storage_perm_remote = false; /* Storage VM is permanently connected to request VM */
	bool     storage_serialized = true; /* Only one storage call at a time */
	bool     double_buffered = false; /* Use double-buffering for resets */
	...
	bool     print_stdout = true; /* Print directly to stdout */
	bool     verbose = false;
	bool     verbose_syscalls = false;
	bool     verbose_pagetable = false;
	int      profiling_interval = 0; /* Samples, 0 = off, 1..N = print and reset */
	std::string cold_start_file; /* File to use for fast cold start */
	...
	std::shared_ptr<std::vector<std::string>> storage_arguments = nullptr;
...
struct TenantConfig
{
	...
	std::string    storage_filename; /* Optional storage program filename */
	std::string    key;
	...
	std::string storage_program_filename() const noexcept {
		if (!this->storage_filename.empty())
			return this->storage_filename;
		/* Default to the main program name with _storage suffix */
		return this->filename + "_storage";
	}
	...
	uint64_t max_storage_memory() const noexcept {
		if (group.max_storage_memory == 0)
			return group.max_main_memory;
		return group.max_storage_memory;
	}
	...
	TenantConfig(std::string nm, std::string fname, std::string storage_fname,
		std::string key, TenantGroup grp, std::string uri);
	~TenantConfig();
