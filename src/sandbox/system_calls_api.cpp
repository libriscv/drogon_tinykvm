...
static void syscall_ipre_resume_storage(vCPU& cpu, MachineInstance& inst)
{
	...
	auto& storage = *inst.program().storage().storage_vm;
	storage.reset_wait_for_requests();
	// Copy buffer, len
	const uint64_t src = cpu.registers().rdi;
	const uint64_t len = cpu.registers().rsi;
	if (src != 0x0 && len > 0) {
		auto& storage_vm = storage.machine();
		if (storage_vm.registers().rdi == 0x0)
			throw std::runtime_error("The remote was not accepting data buffer");
		storage_vm.copy_from_machine(storage_vm.registers().rdi,
			cpu.machine(), src, len);
	}

	inst.machine().ipre_remote_resume_now(inst.tenant().config.max_storage_time());
	if (!storage.is_waiting_for_requests())
		throw std::runtime_error("ipre_resume_storage: Storage did not stop correctly");
}
