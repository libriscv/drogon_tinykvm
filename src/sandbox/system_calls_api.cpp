...
static void syscall_ipre_resume_storage(vCPU& cpu, MachineInstance& inst)
{
	if (inst.is_storage())
		throw std::runtime_error("ipre_resume_storage: Cannot be called from storage");
	auto& remote = cpu.machine().remote();
	MachineInstance& storage = *remote.get_userdata<MachineInstance>();
	ScopedDuration cputime(storage.stats().request_cpu_time);

	// Remember buffer address and length values
	const uint64_t src = cpu.registers().rdi;
	const uint64_t len = cpu.registers().rsi;

	if (inst.tenant().config.group.storage_perm_remote) {
		// Resume storage VM, which is already connected to us
		auto& regs = remote.registers();
		remote.copy_to_guest(regs.rdi, &src, sizeof(src));
		regs.rax = len;
		remote.set_registers(regs);

		storage.stats().invocations++;
		storage.reset_wait_for_requests();
		// Resume storage VM, which is already connected to us
		remote.ipre_permanent_remote_resume_now(true);
		return;
	} else if (inst.tenant().config.group.storage_1_to_1) {
		storage.stats().invocations++;
		storage.reset_wait_for_requests();

		// Resume fixed storage VM
		cpu.machine().ipre_remote_resume_now(false,
		[src, len](tinykvm::Machine& m)
		{
			auto& regs = m.registers();
			m.remote().copy_to_guest(regs.rdi, &src, sizeof(src));
			regs.rax = len;
	...
	[src, len](tinykvm::Machine& m)
	{
		...
		MachineInstance& storage = *m.remote().get_userdata<MachineInstance>();
		storage.stats().invocations++;
		storage.reset_wait_for_requests();
	});
}
