...
extern "C"
int kvm_self_request(const char *c_path, const char *arg, backend_result *result)
{
	...
	if (res != CURLE_OK) {
		...
		if (curl_ctype) {
			...
			char *ctype = new char[result->tsize + 1];
...
extern "C"
int kvm_set_self_request(const char *uri, const char *prefix,
