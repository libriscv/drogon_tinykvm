...
void ProgramInstance::begin_initialization(TenantInstance *ten, bool debug)
{
	try {
		...
		if (this->has_storage())
		{
			...
			if (storage_base_gigapage > 0)
			{
				...
				if (ten->config.group.storage_serialized) {
					...
					storage().storage_vm->machine().cpu().remote_serializer =
						&storage().m_async_mtx;
				}
			}
		}
		...
		printf("Program '%s' is loaded (%s, %s, vm=%zu%s, nodes=%d, storage=%s, huge=%d/%d, ready=%.2fms%s)\n",
			main_vm->name().c_str(),
...
{
	ScopedDuration cputime(storage().storage_vm->stats().request_cpu_time);
	/* Detect wrap-around */
