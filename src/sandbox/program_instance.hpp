...
struct VMPoolItem {
	...
	void reset();
	void deferred_reset(); // Does not put the slot back in the queue
};

struct Reservation {
	VMPoolItem* slot;
	void (*free) (VMPoolItem*);
};
...
public:
	...
	std::vector<std::unique_ptr<MachineInstance>> storage_vm;
	...
	MachineInstance& front_storage() {
		if (main_vm == nullptr)
			throw std::runtime_error("No storage VM");
		return *main_vm;
	}
};
...
public:
	...
	ProgramInstance(
		...
		BinaryStorage storage_binary,
		TenantInstance*, bool debug = false);
	...
	/* Entries are function addresses in an array that belongs
	   ...
	   such as GET and POST. We don't need to know function names because
	   the function just directly registers the address of the function.
	...
	Reservation reserve_vm(TenantInstance*, std::shared_ptr<ProgramInstance>);
	/* Free a reserved VM. This can potentially finish a program. */
	static void vm_free_function(VMPoolItem*);
	...
	long live_update_call(gaddr_t func, ProgramInstance& new_prog, gaddr_t newfunc);
	...
	/* Live debugging feature using the GDB RSP protocol.
	   ...
	   to the server using remote GDB. Once the connection is concluded KVM
	   will continue running, finishing the request normally. */
...
private:
	void begin_initialization(TenantInstance *, bool debug);
	/* Wait for server to listen and this program to complete initialization. */
